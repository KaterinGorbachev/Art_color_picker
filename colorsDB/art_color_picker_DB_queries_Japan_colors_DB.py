...
def show_palletes_for_color(my_pointer, my_con): 
    query_palletes = 'SELECT * FROM palletes_has_colors WHERE palletes_number = ?;'
    palletes_number = 0xFBE6A0   #change to user input 

    my_pointer.execute(query_palletes, palletes_number)

    palletes = my_pointer.fetchone()
